...
def load_documents(directory):
    """
    Traverses the directory, loads all Markdown files,
...
def main():
    ...
    random_count = min(5, len(all_chunks))
    random_chunks = random.sample(all_chunks, random_count)
    output_md = os.path.join(directory, 'random_chucks.md')
    with open(output_md, 'w', encoding='utf-8') as md_file:
        md_file.write(f"# Random Chunks Preview ({random_count})\n\n")
        for i, c in enumerate(random_chunks, 1):
            md_file.write(f"## Random Chunk #{i} | ID: {c['chunk_id']}\n\n")
            md_file.write(c['text'] + "\n\n")
            md_file.write("---\n\n")
    print(f"\nWrote {random_count} random chunks to: {output_md}")
    
    # Write the selected random chunks to a JSON file (overwrites existing data)
    out_file = os.path.join(directory, 'processed_chunks.json')
    with open(out_file, 'w', encoding='utf-8') as f:
        json.dump(random_chunks, f, indent=4, ensure_ascii=False)
    print(f"\nSuccessfully wrote {random_count} random chunks to: {out_file}")
